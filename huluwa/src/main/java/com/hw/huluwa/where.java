	...
	public void set(lives one)
	{
		...
		if(this.on!=null)
		{
			...
			{	
				//this.on.cleanPos();
				this.on.death();
				String w=one.name()+"VS"+this.on.name()+System.getProperty("line.separator")+"结果:"+one.name()+"胜利!"+System.getProperty("line.separator");
				Config.text+=w;
				//System.out.println(one.name()+"VS"+this.on.name()+" 结果:"+one.name()+"胜利！");
			}
			else {
				String w=one.name()+"VS"+this.on.name()+System.getProperty("line.separator")+"结果:"+on.name()+"胜利！"+System.getProperty("line.separator");
				Config.text+=w;
